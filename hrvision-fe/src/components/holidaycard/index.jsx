...
const HolidayCard = ({ cardData, month }) => {
  return (
    <div className="h-[270px] w-72 rounded-lg border border-gray-100 bg-white p-5 shadow-sm hover:shadow-lg overflow-auto ">
      <div className="mt-2 text-lg font-semibold text-gray-600">{month}</div>
      {cardData && cardData.length > 0 ? (
        <ul className="mt-4">
          {cardData.map((data, index) => (
            <li className="mb-2 flex" key={index}>
              <div className="mr-6 flex w-6 flex-col items-center justify-center">
                <span className="text-lg font-medium">
                  {new Date(data.date).toDateString().slice(7, 10)}
                </span>
                <span className="text-sm font-thin text-gray-500 ">
                  {new Date(data.date).toDateString().slice(0, 3) === "Wed"
                    ? new Date(data.date)
                        .toDateString()
                        .slice(0, 3)
                        .toLowerCase()
              ...
              <div className="mt-1">
                <span className="capitalize text-base text-gray-600">{data.name}</span>
              </div>
            </li>
          ))}
        </ul>
      ) : (
        <div className="no-holidays mt-4 flex flex-col items-center justify-center">
          <svg
