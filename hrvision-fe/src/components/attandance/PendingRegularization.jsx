...
import { PendingRegularizationHeader } from "../Dashboard/default/variables/columnsData";
...
const PendingRegularization = () => {
  const columns = PendingRegularizationHeader;
  ...
  useEffect(() => {
    const fetchLeaveData = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_BASE_API_URL}forgetattendance`, {
          headers: {
            "jwt-token": authToken,
          },
        });
    ...
    <div className="h-[75vh] overflow-y-hidden">
      <div className="mt-8 overflow-x-scroll overflow-y-hidden lg:overflow-hidden">
        ...
        >
                      ...
                      >
                        {isReasonColumn ? (
                          <div className="group relative">
                            ...
                            <div className="absolute bottom-full mb-2 w-64 rounded bg-gray-800 p-2 text-xs text-white opacity-0 transition-opacity duration-300 group-hover:opacity-100">
                              {cell.value}
                            </div>
                          </div>
                        ) : (
          ...
          <NoDataFound
            text={
              "It's empty here! No pending regularization requests found." 
            }
            img={NoDataImage}
          />
        )}
