...
import { columnsLeaveApplyDashboard } from "../variables/columnsData";
import {
  useGlobalFilter,
  usePagination,
  useSortBy,
...
const LeaveApprovalTable = () => {
  ...
  const columns = columnsLeaveApplyDashboard;
  ...
  const fetchLeaveData = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BASE_API_URL}leave/pending/all`,
        {
          headers: {
            "jwt-token": authToken,
          },
                ...
                <tr {...headerGroup.getHeaderGroupProps()} key={index}>
                  {headerGroup.headers.map((column, index) => (
                    <th
                      ...
                      className="min-w-[140px] border-b border-gray-200 md:pr-14 xl:pr-16 pb-[10px]  text-center tracking-wider dark:!border-navy-700"
                      key={index}
                    >
                      ...
                      if (cell.column.Header === "NAME") {
                        data = (
                          <div className="flex min-w-[140px] items-center gap-2 pl-6 md:pl-4 lg:pl-0 3xl:pl-4">
                            <SelectButton path={"/allleave/manageleave"} />
                        ...
                        data = (
                          <div className="max-w-[140px] pl-12 md:pl-10 lg:pl-5 3xl:pl-8">
                        ...
                        data = (
                          <div className="min-w-[140px] pl-8 md:pl-4 lg:pl-0 3xl:pl-4">
                            <p className=" text-sm font-bold text-navy-700 dark:text-white">
                              {" "}
                        ...
                        data = (
                          <div className="min-w-[140px] pl-16 md:pl-12 lg:pl-8 3xl:pl-12">
                            <p className="text-sm  font-bold text-navy-700 dark:text-white">
                              {cell.value}
                            </p>
                          </div>
